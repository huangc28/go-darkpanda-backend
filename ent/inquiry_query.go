...
func (iq *InquiryQuery) QueryUsers() *UserQuery {
...
func (iq *InquiryQuery) WithUsers(opts ...func(*UserQuery)) *InquiryQuery {
	query := &UserQuery{config: iq.config}
...
func (iq *InquiryQuery) sqlAll(ctx context.Context) ([]*Inquiry, error) {
	...
	if query := iq.withUsers; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].inquirer_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "inquirer_id" returned %v`, n.ID)
			}
			for i := range nodes {
