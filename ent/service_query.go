...
func (sq *ServiceQuery) QueryCustomer() *UserQuery {
...
func (sq *ServiceQuery) QueryServiceProvider() *UserQuery {
	query := &UserQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (sq *ServiceQuery) WithCustomer(opts ...func(*UserQuery)) *ServiceQuery {
	query := &UserQuery{config: sq.config}
...
func (sq *ServiceQuery) WithServiceProvider(opts ...func(*UserQuery)) *ServiceQuery {
	query := &UserQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withServiceProvider = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context) ([]*Service, error) {
	var (
		...
		loadedTypes = [2]bool{
			sq.withCustomer != nil,
			sq.withServiceProvider != nil,
		}
	)
	if sq.withCustomer != nil || sq.withServiceProvider != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withServiceProvider; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Service)
		for i := range nodes {
			if fk := nodes[i].service_provider_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "service_provider_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ServiceProvider = n
			}
		}
	}
